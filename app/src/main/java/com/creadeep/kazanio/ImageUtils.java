...
public class ImageUtils {
  ...
  public static Bitmap cropImageUntilEnd(Bitmap source, RectF location, Integer verticalOffset){ // crops until the right edge
    Matrix matrix = new Matrix();
    matrix.postRotate(90);

    // Omit negative values
    location.set(Math.max(0f, location.left), Math.max(0f, location.top), Math.max(0f, location.right), Math.max(0f, location.bottom)); // eliminate negative values

    return Bitmap.createBitmap(source, Math.round(location.left)+verticalOffset, 0, Math.round(location.right) - Math.round(location.left)-verticalOffset, Math.min(Math.round(location.bottom), source.getHeight()), matrix, true);
  }

  public static Bitmap cropImage(Bitmap source, RectF location, Integer verticalOffset, Matrix rotate){ // crops until the right edge

    // Omit negative values
    location.set(Math.max(0f, location.left), Math.max(0f, location.top), Math.max(0f, location.right), Math.max(0f, location.bottom)); // eliminate negative values

    return Bitmap.createBitmap(source, Math.round(location.left)+verticalOffset, Math.round(location.top), Math.round(location.right) - Math.round(location.left)-verticalOffset, Math.round(location.bottom) - Math.round(location.top), rotate, true);
  }

  public static String recognizeText(TextRecognizer mTextRecognizer, Context context, Bitmap source){
    ...
    for (int index = 0; index < textBlocks.size(); index++) {
      ...
      for (Text line : tBlock.getComponents()) {
        ...
        for (Text element : line.getComponents()) {
          ...
          resultString = resultString + element.getValue();
        }
      }
    }
